var itemOut = angular.module("itemOut", []);
var myApp = angular.module("myApp", []);

itemOut.controller("itemOutCtrl", function ($scope, $http) {
//    $scope.oid = "";
    $scope.reason = "EVENT";
    $scope.itemOutDate = new Date();
//    $scope.finalTotal = 0;
    ...
    $scope.removeItem = function (item) {
        $scope.removableItem = item;
        $scope.value = -1;
        //alert($scope.cartProduct.indexOf($scope.rClickedProduct));
        $scope.value = $scope.cartItem.indexOf($scope.removableItem);
        if ($scope.value >= 0) {
            $scope.cartItem.splice($scope.value, 1);
        }
    };

    //increase and decrease cart item qty
    $scope.increaseQty = function (item) {
        //alert("hi");
        $scope.qtyChangedItem = item;
        $scope.qtyChangedItem.cartQty = parseInt($scope.qtyChangedItem.cartQty) + 1;
        for (var i = 0; i < $scope.cartItem.length; i++) {
            if ($scope.cartItem[i].itemId === $scope.qtyChangedItem.itemId) {
                $scope.cartItem[i].cartQty = $scope.qtyChangedItem.cartQty;
                $scope.cartItem[i].subTotal = $scope.qtyChangedItem.cartQty;
            }
        }

        $scope.finalTotal = 0;
        for (var i = 0; i < $scope.cartItem.length; i++) {
            $scope.finalTotal = $scope.finalTotal + ($scope.cartItem[i].cartQty);
        }
    };
    ...
    $scope.decreaseQty = function (item) {
        $scope.qtyChangedItem = item;
        $scope.qtyChangedItem.cartQty = parseInt($scope.qtyChangedItem.cartQty) - 1;
        if ($scope.qtyChangedItem.cartQty < 1) {
            $scope.qtyChangedItem.cartQty = 1;
        }
        for (var i = 0; i < $scope.cartItem.length; i++) {
            if ($scope.cartItem[i].itemId === $scope.qtyChangedItem.itemId) {
                $scope.cartItem[i].cartQty = $scope.qtyChangedItem.cartQty;
